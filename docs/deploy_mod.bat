@echo off
echo Deploying mod to Project Zomboid workshop...

REM Get the current directory (where this batch file is located)
set "SOURCE_DIR=%~dp0..\QuickUseDrugs"

REM Set the destination directory (Project Zomboid workshop folder)
set "DEST_DIR=C:\Users\jeffw\Zomboid\Workshop\QuickUseDrugs"

REM Check if source directory exists
if not exist "%SOURCE_DIR%" (
    echo ERROR: Source directory not found: %SOURCE_DIR%
    echo Make sure this batch file is in the same directory as the mod folder
    pause
    exit /b 1
)
...
if %ERRORLEVEL% EQU 0 (
    echo.
    echo SUCCESS: Mod has been deployed to:
    echo %DEST_DIR%
    echo.
